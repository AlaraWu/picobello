...
__thread volatile const double InvLn2N = 0x1.71547652b82fep+0 * N;
__thread volatile const double SHIFT = 0x1.8p+52;
__thread volatile const double C[4] = {0x1.c6af84b912394p-5 / N / N / N,
...
// b = exp(a)
// each time four elements
static inline void vexpf_baseline(float *a, float *b, uint64_t *T) {
    double comp_a_ptr[4];
    double comp_b_ptr[4];
    for (int i = 0; i < 4; i++) {
        comp_a_ptr[i] = (double)a[i];
    }
    uint64_t ki[4], t[4];
    ...
    asm volatile(
        ...
        // clang-format on
        : [ output0 ] "=f"(comp_b_ptr[0]),
            [ output1 ] "=f"(comp_b_ptr[1]),
            [ output2 ] "=f"(comp_b_ptr[2]),
            [ output3 ] "=f"(comp_b_ptr[3])
        : [ input0 ] "f"(comp_a_ptr[0]),
            [ input1 ] "f"(comp_a_ptr[1]),
            [ input2 ] "f"(comp_a_ptr[2]),
            [ input3 ] "f"(comp_a_ptr[3]),
